...
class Fornecedor_model extends Database
{
	private $data;
	private $conn;
	public  $fornecedor;
	...
	public function insert($data)
	{
		...
		$sql = $this->db->query("INSERT INTO  eg_fornecedor($cols) VALUES ($rows);");
	}
	...
	{	
		foreach($data as $column => $value){
			echo $column;
			$sql = $this->db->query("UPDATE  eg_fornecedor SET $column = '$value' WHERE cat_id = '$id' ");
		}
	...
	{
		$sql = $this->db->query("DELETE FROM eg_fornecedor WHERE cat_id = $id_prod");
	}

	public function get($id_prod = NULL)
	{
		if(!isset($id_prod)){
			$sql = $this->db->query("SELECT * FROM eg_fornecedor");
			foreach ($sql as $key) {
			...
			$sql = $this->db->query("SELECT * FROM eg_fornecedor WHERE cat_id = $id_prod");
